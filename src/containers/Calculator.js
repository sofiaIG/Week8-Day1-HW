...
function App() {
  ...
  const divide = (number) => {
    const previousTotalNumber = parseFloat(previousTotal)
    const numberDivision = parseFloat(number)
    if (previousTotalNumber==0 || numberDivision == 0){
      setRunningTotal(parseFloat(0));
    }else{
      setRunningTotal(parseFloat(previousTotal) / parseFloat(number));
    }
  }
